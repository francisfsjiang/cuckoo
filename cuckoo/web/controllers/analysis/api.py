...
class AnalysisApi(object):
    ...
    def tasks_recent(request, body):
        ...
        for row in cursor:
            ...
            tasks[len(tasks)] = {
                ...
                "status": "reported",
                "normal_score": info.get("score"),
                "ia_score": ia_score,
                "ia_score_name": ia_score_name,
                "badge_type": badge_type,
                "target_type": target_type,
                "source_from": "From: {src_ip}:{src_port}".format(**source),
                "source_to":   "To: {dst_ip}:{dst_port}".format(**source)
            }
        return JsonResponse({
            "tasks": sorted(
    ...
    def statistics_month(requests):
        # current_date = datetime.datetime(2017,6,29)
...
def gen_statistics(date):
    ...
    for doc in cursor:
        score = doc.get("info", {}).get("ia_score", 0)
        score = float(score)
        if score < 4:
