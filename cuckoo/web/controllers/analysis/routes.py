...
class AnalysisRoutes:
    ...
    def recent_page(request):
        ...
        class Options:
            ...
            def select_all(self):
                self.file = self.url = self.low = self.high = True
        ...
        if request.GET.get("filter-low", "off") == "on":
            filter_score.append({"info.ia_score": {"$gte": 0, "$lt": 4}})
        else:
            filter_options.low = False
        if request.GET.get("filter-high", "off") == "on":
            filter_score.append({"info.ia_score": {"$gte": 4, "$lte": 10}})
        else:
        ...
        for analysis in cursor:
            ...
            a.ia_score = info.get("ia_score", 0.0)
            if a.ia_score < 4:
                a.ia_score_name = "低危"
            else:
                a.ia_score_name = "高危"
        ...
        analysis_list.sort(key=lambda a : a.id, reverse=True)
